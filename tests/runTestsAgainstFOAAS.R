...
#str(bag(name=name, from=from));q()
    ...
    stopifnot(getRandomFO(name=name, from=from)  ==
              structure("Someone, Thou clay-brained guts, thou knotty-pated fool, thou whoreson obscene greasy tallow-catch! - Me\n", class = "rfoaas"))
              #structure("Fuck you, asshole. - Me", class = "rfoaas"))

    set.seed(42)
    stopifnot(getRandomFO(name=name, from=from) ==
              structure("Fuck me gently with a chainsaw, Someone. Do I look like Mother Teresa? - Me\n", class = "rfoaas"))
