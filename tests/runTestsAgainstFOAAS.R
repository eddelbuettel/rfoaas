...
##
##  Copyright (C) 2015-2026  Dirk Eddelbuettel <edd@debian.org>
##
...
#str(king        (name=name, from=from));q()
    ...
    stopifnot(bye         (from=from)             == "Fuckity bye-bye! - Me")
    ...
    #stopifnot(bag         (name=name, from=from)  == "Eat a bag of dicks, Someone. - Me")
    ...
    stopifnot(getRandomFO(name=name, from=from)  ==
              #structure("Someone, Thou clay-brained guts, thou knotty-pated fool, thou whoreson obscene greasy tallow-catch! - Me", class = "rfoaas"))
              structure("Fuck you, asshole. - Me", class = "rfoaas"))

    set.seed(42)
    stopifnot(getRandomFO(name=name, from=from) ==
              #structure("Fuck me gently with a chainsaw, Someone. Do I look like Mother Teresa? - Me", class = "rfoaas"))
