...
##
##  Copyright (C) 2014-2026  Dirk Eddelbuettel <edd@debian.org>
##
...
.foaas <- function(..., filter=.filter(), language=.language()) {
    ...
    srv <- "https://foaas.io"       				# original service at new URL
...
## ## currently supported at https://foass.1001010.com/
## everyone    <- function(from=.from(), filter=.filter(), language=.language())                      { .foaas("everyone", from, filter=filter, language=language) }
## essex       <- function(name, from=.from(), filter=.filter(), language=.language())                { .foaas("essex", name, from, filter=filter, language=language) }
## london      <- function(name, from=.from(), filter=.filter(), language=.language())                { .foaas("london", name, from, filter=filter, language=language) }
## thanks      <- function(from=.from(), filter=.filter(), language=.language())                      { .foaas("thanks", from, filter=filter, language=language) }
## ity         <- function(name, from=.from(), filter=.filter(), language=.language())                { .foaas("ity", name, from, filter=filter, language=language) }
...
## given       <- function(from=.from(), filter=.filter(), language=.language())                      { .foaas("given", from, filter=filter, language=language) }
## bag         <- function(name, from=.from(), filter=.filter(), language=.language())                { .foaas("bag", name, from, filter=filter, language=language) }
## bowl        <- function(name, from=.from(), filter=.filter(), language=.language())                { .foaas("bowl", name, from, filter=filter, language=language) }
...
## eat         <- function(name, from=.from(), filter=.filter(), language=.language())                { .foaas("eat", name, from, filter=filter, language=language) }
## king        <- function(name, from=.from(), filter=.filter(), language=.language())                { .foaas("king", name, from, filter=filter, language=language) }
## lawn        <- function(name, from=.from(), filter=.filter(), language=.language())                { .foaas("lawn", name, from, filter=filter, language=language) }
...
getRandomFO <- function(name, from=.from(), filter=.filter(), language=.language()) {
    ...
    name_from <- c(
        ...
        ##
        ## -- interim
        ## "essex", "london", "ity", "linus",  "bag", "bowl", "shakespeare", "you", "chainsaw", "eat", "king", "lawn", "off", "donut"
    )
    ...
    just_from <-  c(
        ...
        ##
        ## -- interim
        ## "everyone", "thanks", "pink", "given", "everything", "life", "that", "this"
    )
